...
import { Bookmark, Comment, Follow, Like, NewsFeed, Notification, Post, User } from '../../../schemas/index.js';
import { ENotificationType } from '../../../schemas/NotificationSchema.js';
import { EPrivacy } from '../../../schemas/PostSchema.js';
import { PostService } from '../../../services/index.js';
import { deleteImageFromStorage, multer, uploadImageToStorage } from '../../../storage/cloudinary.js';
    ...
    async (req, res, next) => {
        try {
            ...
            if (agg.length <= 0 && offset === 0) {
                return next(ErrorHandler(404, `${username} hasn't posted anything yet.`));
            } else if (agg.length <= 0 && offset >= 1) {
                return next(ErrorHandler(404, 'No more posts.'));
            }
...
router.get(
    ...
    async (req, res, next) => {
        try {
            ...
            if (!exist) return next(ErrorHandler(400, 'Post not found.'));
            ...
            if (likers.length === 0 && offset < 1) {
                return next(ErrorHandler(404, 'No likes found.'));
            }

            if (likers.length === 0 && offset > 0) {
                return next(ErrorHandler(404, 'No more likes found.'));
            }
